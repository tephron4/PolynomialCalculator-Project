...
public class PolyCalcGUI
{
    ...
    /** A variable for the line to be evaluated */
    String eLine = "";
    
    /**
     * A PolyCalcGUI constructor
     */
    public PolyCalcGUI(){
    ...
    public static void main(String[] args){
        final PolyCalcGUI pc = new PolyCalcGUI();
        JFrame window = new JFrame("Polynomial Calculator");
        window.setSize(500,350);
        window.setLocationRelativeTo(null);
        JPanel content = new JPanel();
        window.setContentPane(content);
        JLabel text = new JLabel("Evaluate: ");
        content.add(text);
        JTextField tInput = new JTextField(20);
        content.add(tInput);
        JButton evalButton = new JButton("EVAL");
        content.add(evalButton);
        JTextArea hist = new JTextArea(15,20);
        content.add(hist);
        hist.setEditable(false);
        // Allows for enter key to be pressed in place of evalButton
        // can enter "wipe" to clean the history (for both the enter key and evalButton)
        tInput.addKeyListener(new KeyAdapter(){
            public void keyPressed(KeyEvent e){
                if(e.getKeyCode() == KeyEvent.VK_ENTER){
                    if(!tInput.getText().toLowerCase().equals("wipe")){
                        String res = pc.evalLine(tInput.getText());
                        hist.append(tInput.getText() + " = " + res + "\n");
                        tInput.setText("");
                    }
                    else{
                        hist.setText("");
                        tInput.setText("");
                        pc.polynomials = new TreeMap<Character,Polynomial>();
                    }
                }
            }
        });
